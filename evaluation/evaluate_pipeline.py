...
# Ensure we import the local app.py and config.py from workspace root
...
def run_benchmark(dataset_path: str, limit: int = None):
    ...
    manual_review_out = os.path.abspath(os.path.join(os.path.dirname(__file__), "../datasets/evaluation/manual_review.csv"))
    df_manual_review.to_csv(manual_review_out, index=False)
    print(f"Saved {manual_review_out}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default=os.path.abspath(os.path.join(os.path.dirname(__file__), "../datasets/evaluation/benchmark_dataset_100.json")),
