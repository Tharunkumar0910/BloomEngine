...
import config
from core.spacy_utils import get_spacy_doc
...
class QuestionUnderstandingEngine:
    ...
    def build_profile(cls, raw_text: str, source_bloom: Optional[str] = None) -> QuestionProfile:
        ...
        from validation.concept_validator import extract_concepts_with_compounds
        concepts, _ = extract_concepts_with_compounds(doc)

        # 4. Extract Technical Entities
        from validation.entity_validator import detect_entities
        entities = tuple(sorted(detect_entities(doc)))

        # 5. Extract Numbers
        from validation.number_validator import extract_numbers_and_standards
